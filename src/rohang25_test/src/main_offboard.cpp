...
public:
	...
	{	
		...
		if (precision_control_mode == 0)  // Pixel
		{
			...
			param[2] = 240; // goal position - Y axis(720.0) 
		}
		else  // NED
		{
...
void OffboardControl::publish_trajectory_setpoint()	
{
	...
	if (precision_control_mode == 0)  // Pixel
	{
		current_object_ = current_object_ned;  // Kalman filtered Pixel -> current_object_ned
			...
			if(detection_flag == true){  // if valid detection
				...
				if(-local_pose.z > 5.0){
					...
					if(log_counter_ % 20 == 0){  // every 2 seconds
						set = {WPT[i][0], WPT[i][1], set[2] - 2}; // descend 3m
						set_position(pose, set);
					}
				}
				else{
	...
	save_object_pixel(PIXEL_FILE_PATH, current_timestamp_, -local_pose.z, yolo_score, current_object_.x, current_object_.y);
